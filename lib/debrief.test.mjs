...
describe('registarIncidente autónomo', () => {
  it('regista precisaRevisaoPIC sem exigir clique do PIC', () => {
    ...
    const jev = {
      answers: {
        acaoMissao: { choice: 'desviar_alternativo', probabilities: { desviar_alternativo: 0.59, prosseguir: 0.2, orbitar: 0.21, regressar_base: 0, abortar_emergencia: 0 } },
        destinoPreferido: { choice: 'stol_proximo' },
        precisaRevisaoPIC: { probability: 0.71 },
      },
    };
    const reg = registarIncidente({
      estado,
      incidente: frente,
      jev,
      baseline: { answers: decisaoGeometrica(estado) },
      pic: { autonomo: true, pediria_pic: true, oferecido: true, forcado: false, sobreposto: false },
    });
    assert.equal(reg.pic.autonomo, true);
    assert.equal(reg.pic.sobreposto, false);
    assert.equal(jev.answers.precisaRevisaoPIC.probability, 0.71);
    assert.equal(reg.max_prob, 0.59);
  });
});
...
describe('resumirMissao', () => {
  ...
  it('no MEDEVAC a regra prossegue na frente meteorológica e o JEV pode provar a tese', () => {
    ...
    assert.match(resumo.veredicto, /desviar|STOL|céu|regra|hospital/i);
