...
export function perguntasTatico(estado) {
  ...
  return {
    ...
    ...(new Set(ameacas.map((a) => a.id)).size >= 2 ? {
      ameacaPrioritaria: {
        type: 'choice',
        instructions:
          CONTEXTO_TATICO +
...
// O contexto completo só vai na manobra, que pesa as categorias de 7 candidatas. As outras
// perguntas levam este: em 30 estados do voo gravado deram as mesmas respostas, com
// 18 % menos tokens por chamada.
const CONTEXT_PILOT_SHORT =
  'You are JEV flying the LUS-222 in real time over Porto on São João night. ' +
  'The state already holds the maths as categories; a deterministic supervisor vetoes collisions. ';
...
/**
 * Cada opção leva as suas categorias (só valores das listas, filtrados no
 * servidor): o JEV compara opção com opção em vez de cruzar a tabela do estado.
 */
function opcaoManobra(m, x = {}) {
  const extras = [
    `separation ${x.separacao ?? 'folgada'}`,
    x.regra_do_ar && x.regra_do_ar !== 'neutra' ? `rule of the air ${x.regra_do_ar}` : null,
    x.nuvem === 'entra' ? 'enters cloud' : null,
    x.terreno === 'perto' ? 'near terrain' : null,
    `route ${x.rota ?? 'mantém'}`,
    `altitude ${x.altitude ?? 'mantém'}`,
  ];
  return `${MANOEUVRE_EN[m]} — ${extras.filter(Boolean).join(' · ')}`;
}

// Prioridades como eliminação. «Manter a manobra em curso» fazia o JEV orbitar:
// agarrava-se à curva com a rota a pedir o lado contrário.
const MANOEUVRE_STEPS =
  'Choose the manoeuvre for the next moment by elimination. Step 1: drop every option with separation «conflito»; if any option is «folgada», also drop the «marginal» ones. ' +
  'Step 2: drop options that break the rule of the air, enter cloud or go near terrain, unless that empties the list. ' +
  'Step 3: among the rest, the best is the one whose route is «aproxima»; if none, «mantém». Step 4: on a tie, prefer altitude «corrige», then «mantém». ' +
  "Honour the commander's orders when they do not break steps 1-2.";
...
export function perguntasPiloto(estado) {
  ...
  return {
    ...
    manobra: {
      type: 'choice',
      instructions: CONTEXT_PILOT + MANOEUVRE_STEPS,
      criteria: Object.fromEntries(manobras.map((m) => [m, opcaoManobra(m, estado.manobras[m])])),
    },
    potencia: {
      type: 'choice',
      instructions:
    ...
    urgencia: {
      type: 'score',
      instructions: CONTEXT_PILOT_SHORT + 'Urgency of this moment: how soon the most critical threat arrives and how small the separation gets if nothing changes.',
      criteria: [
