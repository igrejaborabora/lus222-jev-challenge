...
describe('perguntas tácticas', () => {
  it('as opções são só as candidatas do estado, sem prioritária quando não há ameaças', () => {
    ...
    const uma = CASOS_TATICO.find((c) => c.id === 'frente_mesmo_nivel');
    assert.equal(perguntasTatico(uma.estado).ameacaPrioritaria, undefined, 'com uma só ameaça não há escolha a fazer');
  });
