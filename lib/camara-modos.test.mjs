  ...
  it('o evento enquadra avião e ameaça, com a câmara do lado oposto à ameaça', () => {
    ...
    for (const aspecto of [0.46, 16 / 9]) {
      ...
      for (const foco of focos) {
        ...
        for (const ponto of [pose, foco]) {
          const ndc = projectar(cam, ponto, aspecto);
          assert.ok(ndc.z > 0 && Math.abs(ndc.x) < 1 && Math.abs(ndc.y) < 1, JSON.stringify({ aspecto, foco, ponto, ndc }));
        }
      }
    }
  });
