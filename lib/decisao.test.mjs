...
describe('lerEstado', () => {
  it('fixa o tipo LUS-222 e clampeia lixo', () => {
    ...
    assert.equal(e.geometria.obstaculos.length, 4);
...
describe('decisaoGeometrica (cego)', () => {
  ...
  it('as acções do baseline pertencem ao contrato novo', () => {
    ...
    assert.ok(!('manobraVertical' in d));
  });
});

describe('escalação PIC', () => {
  it('escala se a melhor escolha fica abaixo de 0,55', () => {
    const answers = {
      acaoMissao: {
        type: 'choice',
        choice: 'orbitar',
  ...
  it('escala se P(PIC) é alta mesmo com escolha clara', () => {
    const answers = {
      acaoMissao: {
        type: 'choice',
        choice: 'prosseguir',
