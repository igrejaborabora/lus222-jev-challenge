...
describe('perguntas', () => {
  ...
  it('o contexto completo só vai na manobra; as outras perguntas levam o curto', () => {
    const q = perguntasPiloto({ voo: {}, ameacas: [], manobras: { manter: {}, esquerda: {} } });
    assert.match(q.manobra.instructions, /The maths is already done in the state/);
    for (const k of ['plano', 'potencia', 'urgencia', 'foraDoEnvelope']) {
      assert.doesNotMatch(q[k].instructions, /The maths is already done in the state/, k);
      assert.match(q[k].instructions, /^You are JEV flying the LUS-222/, k);
    }
  });
});
