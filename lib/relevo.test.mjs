...
describe('Douro no relevo do Porto', () => {
  ...
  it('a mancha urbana brilha na cidade, apaga-se longe dela e nunca na água', async () => {
    const { alturaTerreno, perfilTerreno, urbanoEm } = await import('../public/src/relevo.js');
    const porto = perfilTerreno('porto');
    const em = (x, z) => urbanoEm(porto, x, z, alturaTerreno(porto, x, z));
    assert.ok(em(2400, 153100) > 0.3, 'Ribeira acesa');
    assert.equal(em(2200, 152700), 0, 'o Douro não brilha');
    assert.ok(em(-3000, 138500) < 0.02, 'longe da cidade quase nada');
    assert.equal(urbanoEm(perfilTerreno('carga'), 0, 0, 50), 0, 'sem cidade no perfil, nada');
  });
});
