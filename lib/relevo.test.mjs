...
describe('Douro no relevo do Porto', () => {
  it('o leito fica abaixo da água e as margens sobem ao relevo natural', async () => {
    ...
    const longe = alturaTerreno(perfil, 2200, 153600);
    const semRio = alturaTerreno({ ...perfil, rio: null }, 2200, 153600);
    assert.equal(longe, semRio, 'a 900 m do rio o relevo não muda');
    const margem = alturaTerreno(perfil, 2200, 152700 + 200);
    assert.ok(margem > alturaTerreno(perfil, 2200, 152700) && margem <= semRio + 1e-9, 'a margem sobe sem ultrapassar o relevo');
  });
});
