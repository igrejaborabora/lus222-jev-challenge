...
import { estadoPiloto, estadoPilotoCompleto, horasDeRelogio, lerEstadoPiloto, limparOrdens, regraDoAr, textoEstado } from './estado-piloto.mjs';
...
describe('filtro do servidor', () => {
  ...
  it('o servidor descarta ids de ameaça repetidos e só aceita estados com duas manobras', () => {
    const a = { id: 'a1', tipo: 'balões de São João', posicao: '12 horas', movimento: 'converge', tempo_ate_cpa: 'curto', cpa_se_manter: 'conflito', altura_relativa: 'mesmo nível', intencao: 'constante' };
    const e = lerEstadoPiloto({ ameacas: [a, { ...a, posicao: '3 horas' }, { ...a, id: 'a2' }], manobras: { manter: {}, esquerda: {} } });
    assert.deepEqual(e.ameacas.map((x) => [x.id, x.posicao]), [['a1', '12 horas'], ['a2', '12 horas']]);
    assert.equal(estadoPilotoCompleto(e), true);
    // Sem manobras só fica «manter»: uma escolha de uma opção, que o Gateway recusa.
    assert.equal(estadoPilotoCompleto(lerEstadoPiloto({})), false);
  });
});
