...
import { alturaAteFolga, emLeitura, indiceAmeacaAEnquadrar, leituraAmeaca } from '../public/src/ameaca-visual.js';
...
describe('ameaça a enquadrar no evento', () => {
  const aviao = { x: 0, z: 0 };

  it('prefere o tipo da leitura, mesmo que outra ameaça esteja mais perto', () => {
    const ameacas = [{ tipo: 'relevo', x: 0, z: 200 }, { tipo: 'bando', x: 0, z: 600 }];
    assert.equal(indiceAmeacaAEnquadrar(ameacas, 'bando', aviao), 1);
  });

  it('entre as do mesmo tipo, a mais próxima do avião (não a primeira)', () => {
    const ameacas = [{ tipo: 'bando', x: 0, z: 900 }, { tipo: 'bando', x: 30, z: 400 }, { tipo: 'bando', x: 0, z: 700 }];
    assert.equal(indiceAmeacaAEnquadrar(ameacas, 'bando', aviao), 1);
  });

  it('sem nenhuma do tipo lido, a mais próxima de todas; sem ameaças, −1', () => {
    const ameacas = [{ tipo: 'relevo', x: 0, z: 800 }, { tipo: 'torre', x: -100, z: 300 }];
    assert.equal(indiceAmeacaAEnquadrar(ameacas, 'bando', aviao), 1);
    assert.equal(indiceAmeacaAEnquadrar([], 'bando', aviao), -1);
    assert.equal(indiceAmeacaAEnquadrar(undefined, 'bando', aviao), -1);
  });
});
