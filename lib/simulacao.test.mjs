...
import { PERFIL, criarMissao, avancarMissao, aplicarDecisao, proximoEvento, estadoParaAvaliacao, pistaNecessariaM, combustivelNecessarioKg } from '../public/src/simulacao.js';
...
describe('simulação causal', () => {
  ...
  it('o supervisor atribui a si a separação não proposta pelo JEV', () => {
    let m = criarMissao('porto', 222);
    while (!proximoEvento(m)) m = avancarMissao(m, 1);
    const r = aplicarDecisao(m, decisao());
    assert.equal(r.supervisor.fonte, 'supervisor');
    assert.match(r.supervisor.motivo, /ameaça iminente/i);
    assert.equal(r.missao.comando.lateral, 'direita');
  });
});
