...
describe('autómato de evasão', () => {
  it('subir e direita: sobe, nariz acima, vira à direita do ecrã', () => {
    ...
    assert.ok(a.y > y0 + 4, `altitude ${a.y} vs ${y0}`);
    assert.ok(a.heading < h0 - 0.08, `heading ${a.heading} vs ${h0}`);
    assert.ok(a.bank > 0.1, `bank ${a.bank}`);
    assert.ok(a.pitch < -0.03, `pitch ${a.pitch}`);
  });
  ...
  it('esquerda no ecrã sobe o heading; descer baixa altitude e o nariz', () => {
    ...
    assert.ok(a.heading > h0 + 0.05, `heading ${a.heading}`);
    assert.ok(a.y < 48, `altitude ${a.y}`);
    assert.ok(a.pitch > 0.015, `pitch ${a.pitch}`);
  ...
  it('o último eixo continua depois da janela de dodge', () => {
    ...
    assert.ok(a.heading < h0 - 0.12, `heading ${a.heading} vs ${h0}`);
    const h = a.heading;
