...
export const CENARIOS_SIM = {
  ...
  sar: {
    ...
    origem: P('origem', 'origem', 0, 0, 1400), planeado: P('planeado', 'planeado', 0, 110000, 1400),
...
export function criarMissao(cenarioId = 'medevac', semente = 222, restricoes = {}) {
  ...
  return {
    ...
    missao: { almas: c.almas, tempoLimiteS: c.tempoLimiteS, prioridade: id === 'carga' ? 'carga_critica' : 'tempo' },
...
export function estadoParaAvaliacao(m, evento = null) {
  ...
  return {
    aeronave: { tipo: 'LUS-222', fuel_kg: Math.round(m.voo.combustivelKg), payload_kg: m.voo.payloadKg, config_cabine: m.cenario === 'medevac' ? 'medevac' : m.cenario === 'sar' ? 'mista' : 'carga', tripulantes: 2, integridade: m.voo.integridade, alcance_restante_km: Math.round(m.voo.combustivelKg / 0.09 * 88 / 1000) },
    ...
    ambiente: { tecto_ft: e.tetoFt ?? m.ambiente.tetoFt, vis_km: e.visKm ?? m.ambiente.visKm, vento_kt: e.ventoKt ?? Math.round(Math.hypot(m.ambiente.ventoMs.x, m.ambiente.ventoMs.z) * 1.944), superficie_pista: destinoDe(m, m.destinoId).superficie, comprimento_pista_m: destinoDe(m, m.destinoId).pistaM, luz_dia: m.ambiente.luzDia },
    geometria: { obstaculos: e.obstaculos ?? [] },
    alternativas: m.destinos.map((d) => ({ id: d.id, tipo: d.tipo, distancia_km: Math.round(dist(m.voo, d) / 1000), pista_m: d.pistaM, superficie: d.superficie, pista_necessaria_m: pistaNecessariaM(m, d), combustivel_necessario_kg: combustivelNecessarioKg(m, d) })),
    restricoes: { ...m.restricoes },
    incidente: { id: e.id, tipo: e.tipo, resumo: e.resumo },
  };
}

export function aplicarDecisao(m, answers, fonte = 'jev') {
  ...
  if (acao === 'regressar_base') { destinoId = 'origem'; fase = 'regresso'; }
    ...
    const viable = m.destinos.filter((d) => d.id !== 'planeado' && d.pistaM >= pistaNecessariaM(m, d) && m.voo.combustivelKg >= combustivelNecessarioKg(m, d));
    ...
    else if (escolhido.pistaM < pistaNecessariaM(m, escolhido)) motivo = 'Pista insuficiente para a massa e vento calculados';
    else if (m.voo.combustivelKg < combustivelNecessarioKg(m, escolhido)) motivo = 'Combustível insuficiente com reserva';
    else { destinoId = preferido; fase = 'desvio'; }
  }
  if (acao === 'regressar_base') {
    const origem = destinoDe(m, 'origem');
    if (m.voo.combustivelKg < combustivelNecessarioKg(m, origem)) motivo = 'Combustível insuficiente para regressar com reserva';
  }
  if (motivo) {
    const validos = m.destinos.filter((d) => d.id !== 'stol_proximo' && d.pistaM >= pistaNecessariaM(m, d) && m.voo.combustivelKg >= combustivelNecessarioKg(m, d));
  ...
  const comando = { acao: motivo ? 'orbitar' : acao, vertical: answers?.manobraVertical?.choice ?? 'manter', lateral: answers?.manobraLateral?.choice ?? 'manter', urgencia: answers?.urgencia?.score ?? 1, evasaoAteS: m.voo.tempoS + 22 };
  return {
    missao: { ...m, destinoId, fase, eventosPendentes: pendentes, eventosTratados: evento ? [...m.eventosTratados, evento.id] : m.eventosTratados, comando, orbitaRestanteS: fase === 'orbita' ? 90 : m.orbitaRestanteS },
    supervisor: { interveio: Boolean(motivo), motivo, proposta: { acao, destino: preferido }, aplicada: { acao: comando.acao, destino: destinoId }, fonte: motivo ? 'supervisor' : fonte },
  };
}
...
function passoFisico(m, dt) {
  ...
  const alvoV = m.fase === 'orbita' ? 72 : m.fase === 'emergencia' ? 78 : 88;
  ...
  const altitudeAlvo = evasao && m.comando.vertical === 'subir' ? 610 : evasao && m.comando.vertical === 'descer' ? 360 : m.fase === 'emergencia' ? 360 : 480;
  ...
  const altitude = Math.max(35, v.altitudeM + subida * dt);
...
export function avancarMissao(m, segundos) {
  ...
  while (restante > 1e-7 && !atual.resultado) {
    ...
    else if (voo.altitudeM <= 35) resultado = 'limite_altitude';
    else if (fase !== 'orbita' && dist(voo, destino) < 900) resultado = atual.destinoId === 'origem' ? 'regressou' : fase === 'emergencia' ? 'emergencia_resolvida' : 'chegou';
